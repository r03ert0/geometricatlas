...
void msh_parseVerticesDataBin(float **vdat, int nd, int np, char *data, int size)
{
    ...
    int		i,j=0;
    char	stop,b4[4],swb4[4];
    int		np_file=getnumfromtxt(&j,data,&stop);
	float	max,min;

    if(np_file!=np)	printf("error: texture dimensions\n");
    *vdat=(float*)calloc(nd*np,sizeof(float));
    for(i=0;i<nd*np;i++)
	{

...
bool em_textureDepth(MeshPtr m, float *C)
{
    ...
    for(i=0;i<(*m).np;i++)
    {
        n=	pow(2*(p[i].x-(*m).center.x)/((*m).bbox.ide.x-(*m).bbox.siz.x),2) +
                pow(2*(p[i].y-(*m).center.y)/((*m).bbox.ide.y-(*m).bbox.siz.y),2) +
                pow(2*(p[i].z-(*m).center.z)/((*m).bbox.ide.z-(*m).bbox.siz.z),2);
